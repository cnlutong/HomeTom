...
\begin{frame}{Implementation: Global Architecture}
    ...
    \begin{tikzpicture}[scale=0.8, transform shape]
        % Zones
        \draw[fill=FUblue!5, dashed] (0,0) circle (3.5cm);
        \draw[fill=white, draw=FUblue, thick] (0,0) circle (2.5cm);
        \draw[fill=FUgreen!10, draw=FUgreen, thick] (0,0) circle (1.5cm);
        
        % Labels
        \node at (0, 2.8) {\bfseries Application Layer};
        \node at (0, 1.8) {\bfseries Domain Layer};
        \node at (0, 3.8) {\bfseries Infrastructure / API};
        
        % Content
        \node[align=center] at (0,0) {\textbf{Core Domains}\\\small Device\\Scene\\Execution};
        \node[align=center] at (0,-2) {\small Orchestration Service\\Workflow Engine};
        \node[align=center] at (3.5, 0) {\small FastAPI\\Routers};
        \node[align=center] at (-3.5, 0) {\small SQLAlchemy\\Persistence};
    \end{tikzpicture}
    
    \vspace{0.5em}
    \small \textbf{Benefit}: Business logic is isolated from external tools (Database, HTTP API).
\end{frame}
...
\begin{frame}{Implementation: Technology Stack}
    ...
    \begin{columns}[t]
        \column{0.33\textwidth}
            \textbf{\color{FUblue}Backend}
            \begin{itemize}
                \item \textbf{Language}: Python 3.10+
                \item \textbf{Framework}: FastAPI (Async)
                \item \textbf{Validation}: Pydantic v2
                \item \textbf{ORM}: SQLAlchemy 2.0
            \end{itemize}

        \column{0.33\textwidth}
            \textbf{\color{FUblue}Frontend}
            \begin{itemize}
                \item \textbf{Framework}: React (Vite)
                \item \textbf{UI Lib}: TailwindCSS
                \item \textbf{Editor}: Custom DAG (Reference: n8n)
            \end{itemize}

        \column{0.33\textwidth}
            \textbf{\color{FUblue}Infrastucture}
            \begin{itemize}
                \item \textbf{Ecosystem}: Home Assistant (via API)
                \item \textbf{Mock Server}: Custom Python simulated hardware
                \item \textbf{Dev}: Docker \& Pytest
            \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Solution: User Interface}
    ...
    \centering
    \includegraphics[width=0.85\textwidth]{ui_screenshot.png}
