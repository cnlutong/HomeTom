...
python3 -m uvicorn test_API_server.main:app --port 8123 --reload > /dev/null 2>&1 &
TEST_API_PID=$!

# 2. Start HomeTom Backend (Port 8000)
echo -e "${GREEN}[2/3] Starting HomeTom Backend (Port 8000)...${NC}"
# Wait a few seconds for the mock server to initialize if needed
sleep 2
uvicorn src.main:app --host 0.0.0.0 --port 8000 --reload > /dev/null 2>&1 &
BACKEND_PID=$!
...
echo -e "  • Frontend:    http://localhost:5173 (default Vite port)"
echo -e "${BLUE}============================================================${NC}"
